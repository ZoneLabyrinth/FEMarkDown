...
- loader 直译为“加载器”。webpack将一切文件视为模块，但是webpack原生是只能解析js文件，如果想将其他文件也打包的话，就会用到loader。所以loader的作用是让webpack用友了加载和解析非javascript文件的能力
- plugin直译为“插件”。plugin可以扩展webpack的功能，让webpack具有更多的灵活性。在webpack运行的生命周期中会广播出许多事件。plugin可以监听这些事件，在合适的时机通过webpack提供的api该表输出结果
